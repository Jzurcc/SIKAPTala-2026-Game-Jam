...
func _add_float_animation() -> void:
	...
	var dur = 0.8 # Snappier cycle
	var amt = 2.5 # Subtle but clear
	var base_y = container.position.y
	
	# Move to starting extreme for a seamless loop
	container.position.y = base_y - amt
	
	# Oscillate between peaks
...
var _shake_tweens: Dictionary = {} # Node -> Tween
...
func _on_tag_gui_input(event: InputEvent, node: Control) -> void:
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		if "LOCKED" in current_tags:
			# Find the specific LOCKED tag node and shake it
			for child in container.get_children():
...
func _on_tag_hover(is_hover: bool, node: Control) -> void:
	...
	if is_hover:
		# Quick bouncy overshoot for individual tags
